...
### Elastic Beanstalk
서버 애플리케이션 배포는AWS Elastic Beanstalk을 이용하기로 했다. 가장 쉽게 배포할 수 있는 방법이기도 하고, 환경을 구성하는 것 자체는 과금이 되지 않기 때문이다. 또, AWS에 대해 잘 아는 것이 아니기 때문에 우선 가장 쉽게 배포할 수 있는 방식을 선택하고, 그 이후에 이러한 배포 과정이 어떻게 이루어지는지 추가적으로 공부하는 것이 조금 더 나와 잘 맞는 방식이라는 생각이 들었다.
